...
REV = 'R0.11'
OUT = Path(__file__).resolve().parent / 'MissionComHub_MkI_Enclosure_R0.11'
...
PD_CARRIER_W = 30.0
PD_CARRIER_T = 2.40
PD_CARRIER_CENTER_Y = 25.0
PD_CARD_CENTERS_X = (-27.0, 27.0)
PD_GUIDE_H = 1.60
PD_GUIDE_T = 1.40
PD_ZIP_SLOT_L = 5.0
PD_ZIP_SLOT_W = 2.30
...
FUSE_CENTER_X = 8.0
FUSE_CENTER_Y = 22.1      # I/O half after R0.11 block swap
...
BUS_CENTER_X = 0.0
BUS_CENTER_Y = -24.95     # control half after R0.11 block swap
...
for cx in PD_CARD_CENTERS_X:
    for sy in (-1, 1):
        guide_y = sy * (pd_bay_w / 2 + PD_GUIDE_T / 2)
        guide = (
            cq.Workplane('XY').center(cx, guide_y)
    ...
    for zip_x in (cx - 12.0, cx + 12.0):
        for sy in (-1, 1):
            slot_y = sy * (pd_bay_w / 2 + 2.0)
            zip_slot = (
...
try:
    for label, cx in [('BATTERY PD', PD_CARD_CENTERS_X[0]), ('STARLINK PD', PD_CARD_CENTERS_X[1])]:
        mark = (
            cq.Workplane('XY').workplane(offset=0.01).center(cx, 0)
            .text(label, 3.0, 0.35, font='DejaVu Sans', halign='center', valign='center')
        )
...
parts={
 'MCH-MkI_body_R0.11':body,
 'MCH-MkI_lid_R0.11':lid,
 'MCH-MkI_dual_USB-PD_card_carrier_R0.11':pd_carrier,
 'MCH-MkI_peplink_strain_relief_R0.11':clamp,
 'MCH-MkI_control_fit_coupon_R0.11':control_coupon,
 'MCH-MkI_USB-C_final_fit_coupon_R0.11':usb_coupon,
 'MCH-MkI_wire_exit_fit_coupon_R0.11':wire_coupon,
 'MCH-MkI_BlueSea5045_fused_block_riser_coupon_R0.11':fused_block_coupon,
 'MCH-MkI_unfused_busbar_riser_coupon_R0.11':unfused_block_coupon,
}
...
manifest={
 ...
 'pd_card_carrier': {
   ...
   'card_centers_in_enclosure_xy_mm':[
       [PD_CARD_CENTERS_X[0],PD_CARRIER_CENTER_Y],
       [PD_CARD_CENTERS_X[1],PD_CARRIER_CENTER_Y],
   ],
...
shutil.copy(Path(__file__).resolve(),OUT/'make_mch_mk1_r11.py')
(OUT/'dimensions.json').write_text(json.dumps(manifest,indent=2))

readme=f'''# Mission Communications Hub Mk I Enclosure R0.11 — Tall Swapped-Block Body
...
placement=f'''# USB-PD Card Placement — R0.11
...
- Battery card center: **X {PD_CARD_CENTERS_X[0]:.1f}, Y +{PD_CARRIER_CENTER_Y:.1f} mm**.
- Starlink card center: **X +{PD_CARD_CENTERS_X[1]:.1f}, Y +{PD_CARRIER_CENTER_Y:.1f} mm**.
