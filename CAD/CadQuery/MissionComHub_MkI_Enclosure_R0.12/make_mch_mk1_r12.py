...
REV = 'R0.12'
OUT = Path(__file__).resolve().parent / 'MissionComHub_MkI_Enclosure_R0.12'
...
H = 103.0                 # Z, R0.10 plus approximately 2 inches
WALL = 3.0
BOTTOM = 3.2
CORNER_R = 6.0
LID_T = 4.0
...
SWITCH_DEPTH = 1.5 * 25.4 # 38.10 inward projection measured after R0.10 print
...
# Removable dual USB-PD card carrier. The carrier mounts to four blind,
# heat-set-insert bosses on the underside of the lid. Open-ended guide rails
# and paired zip-tie slots accommodate
# either board orientation without relying on undocumented PCB hole patterns.
PD_CARD_COUNT = 2
PD_CARD_TOTAL_L = PD_BOARD_L + PD_SOLDER_EXT
PD_CARD_CLEARANCE = 0.60
PD_CARRIER_L = 64.0
PD_CARRIER_W = 54.0
PD_CARRIER_T = 2.40
PD_CARRIER_CENTER_Y = 17.0
PD_CARD_CENTERS_Y = (-12.5, 12.5)
PD_GUIDE_H = 3.60
PD_GUIDE_T = 1.40
PD_GUIDE_INSET = 1.00
PD_ZIP_SLOT_L = 5.0
...
PD_LID_BOSS_H = 4.50
PD_LID_BOSS_D = 8.0
PD_INSERT_PILOT_D = 4.60
PD_INSERT_DEPTH = 4.0
PD_CARRIER_SCREW_D = 3.40
PD_ASSUMED_ASSEMBLY_DEPTH = 8.0
PD_USB_PLUG_EXTENSION = 38.10
PD_USB_PLUG_W = 14.0
PD_USB_PLUG_DEPTH = 10.0
PD_TERMINAL_WIRE_SERVICE_L = 20.0
...
USB_SCREW_CTC = 18.55          # confirmed by progressive physical coupons
USB_SCREW_D = 2.8
...
FUSE_CENTER_X = 8.0
FUSE_CENTER_Y = 22.1      # I/O half after R0.12 block swap
...
BUS_MOUNT_CTC = 4.75 * 25.4    # 120.65, confirmed physical fit
BUS_MOUNT_HOLE_D = 5.5
BUS_CENTER_X = 0.0
BUS_CENTER_Y = -24.95     # control half after R0.12 block swap
...
control_y = -W/2
io_y = W/2
# The taller body creates a dedicated upper control/connector zone. Keeping
# the panel centers 26 mm below the lid places the 38.1 mm-deep switch well
# above both power blocks.
panel_z = H - 26.0
...
# Physical unfused bus-bar mounting bosses at confirmed 120.65 mm centers.
...
try:
    for label,x in [('BATTERY',usb_in_x),('STARLINK',usb_out_x),('PEPLINK',peplink_x)]:
        label_z = panel_z - 16.5 if label != 'PEPLINK' else 12.0
...
try:
    for label,x in [('VOLTAGE',meter_x),('POWER',switch_x)]:
        txt = cq.Workplane('XZ', origin=(0, control_y+0.02, 0)).text(
            label, 4.2, 0.65, font='DejaVu Sans', halign='center', valign='center'
        ).translate((x,0,panel_z - 18.5))
        body = body.cut(txt)
except Exception as e:
    print('Control label warning:', e)

# The full-length bus bar now occupies the control half. Move the tall lid
# posts to the I/O half; their X positions remain outside the fused block.
boss_positions=[(-66,0),(66,0),(-66,39),(66,39)]
...
# Vent slots on the short sides at lower and upper elevations.
for xside in (-1,1):
    x=xside*L/2
    for vent_z in (40.0, 76.0):
        for y in (-18,0,18):
            vent=cq.Workplane('YZ').center(y,vent_z).rect(12,3.0).extrude(WALL+3,both=True).translate((x,0,0))
            body=body.cut(vent)
...
# Four blind M3 insert bosses support the removable USB-PD carrier. They extend
# only from the underside, so the branded outer surface remains unchanged.
pd_lid_mount_positions = [
    (-PD_LID_MOUNT_X, PD_CARRIER_CENTER_Y - PD_LID_MOUNT_Y),
    ( PD_LID_MOUNT_X, PD_CARRIER_CENTER_Y - PD_LID_MOUNT_Y),
    (-PD_LID_MOUNT_X, PD_CARRIER_CENTER_Y + PD_LID_MOUNT_Y),
    ( PD_LID_MOUNT_X, PD_CARRIER_CENTER_Y + PD_LID_MOUNT_Y),
]
for x, y in pd_lid_mount_positions:
    pd_boss = (
        cq.Workplane('XY').center(x, y).circle(PD_LID_BOSS_D / 2)
        .extrude(PD_LID_BOSS_H).translate((0, 0, -PD_LID_BOSS_H))
    )
    # Hole opens from the bottom of the boss and remains blind to the lid top.
    pd_insert_hole = (
        cq.Workplane('XY').center(x, y).circle(PD_INSERT_PILOT_D / 2)
        .extrude(PD_INSERT_DEPTH + 0.1)
        .translate((0, 0, -PD_LID_BOSS_H - 0.05))
    )
    lid = lid.union(pd_boss).cut(pd_insert_hole)
...
# ---------------- REMOVABLE USB-PD CARD CARRIER ----------------
# Print rail-side upward. During assembly, rotate the carrier over so its flat
# face seats against the four lid bosses and its card bays face into the body.
pd_carrier = rounded_box(PD_CARRIER_L, PD_CARRIER_W, PD_CARRIER_T, 3.0)

# Four M3 clearance holes align with the lid's blind insert bosses.
for x in (-PD_LID_MOUNT_X, PD_LID_MOUNT_X):
    for y in (-PD_LID_MOUNT_Y, PD_LID_MOUNT_Y):
        pd_carrier = pd_carrier.cut(
            cq.Workplane('XY').center(x, y).circle(PD_CARRIER_SCREW_D / 2)
            .extrude(PD_CARRIER_T + 1.0).translate((0, 0, -0.5))
        )

# Each bay has open-ended side rails. The 45.72 mm length includes the
# user-reported 2.54 mm solder-joint projection opposite the USB connector.
pd_bay_l = PD_CARD_TOTAL_L + PD_CARD_CLEARANCE
pd_bay_w = PD_BOARD_W + PD_CARD_CLEARANCE
for cy in PD_CARD_CENTERS_Y:
    for sy in (-1, 1):
        # Move each rail 1.0 mm inward so it supports the PCB edge instead of
        # allowing the received 17.78 mm board to fall between the rails.
        guide_y = cy + sy * (pd_bay_w / 2 + PD_GUIDE_T / 2 - PD_GUIDE_INSET)
        guide = (
            cq.Workplane('XY').center(0, guide_y)
            .rect(pd_bay_l, PD_GUIDE_T).extrude(PD_GUIDE_H)
            .translate((0, 0, PD_CARRIER_T))
        )
        pd_carrier = pd_carrier.union(guide)

    # Two independent zip ties per card. Slots sit just outside the PCB edges,
    # allowing replacement without committing to a vendor-specific hole pattern.
    for zip_x in (-12.0, 12.0):
        for sy in (-1, 1):
            slot_y = cy + sy * (pd_bay_w / 2 + 2.0)
            zip_slot = (
                cq.Workplane('XY').center(zip_x, slot_y)
                .rect(PD_ZIP_SLOT_L, PD_ZIP_SLOT_W)
                .extrude(PD_CARRIER_T + 1.0).translate((0, 0, -0.5))
            )
            pd_carrier = pd_carrier.cut(zip_slot)

try:
    for label, cy in [('BATTERY PD', PD_CARD_CENTERS_Y[0]), ('STARLINK PD', PD_CARD_CENTERS_Y[1])]:
        mark = (
            cq.Workplane('XY').workplane(offset=0.01).center(0, cy)
            .text(label, 2.5, 0.35, font='DejaVu Sans', halign='center', valign='center')
        )
        pd_carrier = pd_carrier.cut(mark)
except Exception as e:
    print('PD carrier label warning:', e)
...
# Final single-position USB coupon uses the confirmed 18.55 mm spacing.
usb_coupon = rounded_box(36,34,3.2,3)
usb_through = rounded_rect_prism_xy(
    USB_CUT_W,USB_CUT_H,5.2,USB_CUT_H/2-0.10,z0=-1.0
).translate((0,3.0,0))
usb_coupon = usb_coupon.cut(usb_through)
for dx in (-USB_SCREW_CTC/2,USB_SCREW_CTC/2):
    usb_screw = cq.Workplane('XY').center(dx,3.0).circle(USB_SCREW_D/2).extrude(5.2).translate((0,0,-1.0))
    usb_coupon = usb_coupon.cut(usb_screw)
usb_pocket = rounded_rect_prism_xy(
    USB_FLANGE_W+USB_FLANGE_CLEARANCE,
    USB_FLANGE_H+USB_FLANGE_CLEARANCE,
    USB_RECESS_DEPTH+0.2,
    (USB_FLANGE_H+USB_FLANGE_CLEARANCE)/2-0.05,
    z0=3.2-USB_RECESS_DEPTH,
).translate((0,3.0,0))
usb_coupon = usb_coupon.cut(usb_pocket)
try:
    usb_marking = cq.Workplane('XY').workplane(offset=3.2).center(0,-10.5).text(
        '18.55',3.2,0.45,font='DejaVu Sans',halign='center',valign='center'
    )
    usb_coupon = usb_coupon.cut(usb_marking)
except Exception as e:
    print('USB coupon label warning:',e)
...
# Final unfused-bus coupon using confirmed 120.65 mm mounting centers.
...
parts={
 'MCH-MkI_body_R0.12':body,
 'MCH-MkI_lid_R0.12':lid,
 'MCH-MkI_dual_USB-PD_card_carrier_R0.12':pd_carrier,
 'MCH-MkI_peplink_strain_relief_R0.12':clamp,
 'MCH-MkI_control_fit_coupon_R0.12':control_coupon,
 'MCH-MkI_USB-C_final_fit_coupon_R0.12':usb_coupon,
 'MCH-MkI_wire_exit_fit_coupon_R0.12':wire_coupon,
 'MCH-MkI_BlueSea5045_fused_block_riser_coupon_R0.12':fused_block_coupon,
 'MCH-MkI_unfused_busbar_riser_coupon_R0.12':unfused_block_coupon,
}
...
manifest={
 ...
 'control_layout': {
   'panel_center_z_mm':panel_z,
   'switch_inward_depth_mm':SWITCH_DEPTH,
   'switch_center_x_mm':switch_x,
   'voltmeter_center_x_mm':meter_x,
   'control_face_y_mm':control_y,
 },
 'pd_board_mm':[round(PD_BOARD_W,2),round(PD_BOARD_L,2),round(PD_SOLDER_EXT,2)],
 'pd_card_carrier': {
   'card_count':PD_CARD_COUNT,
   'card_envelope_each_mm':[round(PD_CARD_TOTAL_L,2),round(PD_BOARD_W,2)],
   'carrier_mm':[PD_CARRIER_L,PD_CARRIER_W,PD_CARRIER_T],
   'carrier_center_in_enclosure_xy_mm':[0.0,PD_CARRIER_CENTER_Y],
   'card_centers_in_enclosure_xy_mm':[
       [0.0,PD_CARRIER_CENTER_Y + PD_CARD_CENTERS_Y[0]],
       [0.0,PD_CARRIER_CENTER_Y + PD_CARD_CENTERS_Y[1]],
   ],
   'layout':'side_by_side_parallel_usb_positive_x',
   'guide_clearance_total_mm':PD_CARD_CLEARANCE,
   'guide_inset_each_side_mm':PD_GUIDE_INSET,
   'guide_inward_change_total_mm':PD_GUIDE_INSET*2,
   'guide_height_mm':PD_GUIDE_H,
   'guide_height_increase_mm':2.0,
   'retention':'two_zip_ties_per_card',
   'lid_mount_centers_local_mm':[PD_LID_MOUNT_X*2,PD_LID_MOUNT_Y*2],
   'lid_boss_height_mm':PD_LID_BOSS_H,
   'm3_insert_pilot_diameter_mm':PD_INSERT_PILOT_D,
   'm3_insert_depth_mm':PD_INSERT_DEPTH,
   'carrier_clearance_hole_diameter_mm':PD_CARRIER_SCREW_D,
   'assumed_max_card_assembly_depth_mm':PD_ASSUMED_ASSEMBLY_DEPTH,
   'attached_usb_plug_envelope_mm':[PD_USB_PLUG_EXTENSION,PD_USB_PLUG_W,PD_USB_PLUG_DEPTH],
   'terminal_wire_service_length_mm':PD_TERMINAL_WIRE_SERVICE_L,
   'terminal_end':'open_negative_x',
 },
 ...
 'unfused_busbar': {
   ...
   'measured_mount_centers_mm':round(BUS_MOUNT_CTC,2),
   'mount_hole_diameter_mm':BUS_MOUNT_HOLE_D,
   'body_boss_center_mm':[BUS_CENTER_X,BUS_CENTER_Y],
   'status':'confirmed_physical_fit',
 },
 'power_block_layout':'swapped_in_R0.11_busbar_control_half_fused_block_io_half',
...
shutil.copy(Path(__file__).resolve(),OUT/'make_mch_mk1_r12.py')
(OUT/'dimensions.json').write_text(json.dumps(manifest,indent=2))

readme=f'''# Mission Communications Hub Mk I Enclosure R0.12 — Side-by-Side PD Carrier
...
- Body height: {H:.1f} mm, increased by {H-52.0:.1f} mm ({(H-52.0)/25.4:.2f} inches)
- Measured switch inward depth: {SWITCH_DEPTH:.2f} mm
- Control and USB panel center height: {panel_z:.1f} mm
- Unfused bus bar: control half, Y={BUS_CENTER_Y:.2f} mm
- Blue Sea 5045 fused block: I/O half, Y=+{FUSE_CENTER_Y:.2f} mm
- Tall lid posts moved to the I/O half and kept outside the fused-block footprint

## I/O face
- Two measured panel mounts: {USB_FLANGE_W:.1f} x {USB_FLANGE_H:.2f} mm rounded flange, {USB_SCREW_CTC:.2f} mm confirmed screw centers
...
- Confirmed mounting centers: {BUS_MOUNT_CTC:.2f} mm (4.75 inches)
...
## USB-PD card carrier
- Separate removable carrier: {PD_CARRIER_L:.1f} x {PD_CARRIER_W:.1f} x {PD_CARRIER_T:.1f} mm
- Installs under the lid, centered at X=0, Y={PD_CARRIER_CENTER_Y:.1f} mm over the fused-block half
- Two open-ended bays for {PD_CARD_TOTAL_L:.2f} x {PD_BOARD_W:.2f} mm card envelopes
...
- Guide rails move inward {PD_GUIDE_INSET:.1f} mm per side and rise to {PD_GUIDE_H:.1f} mm
- USB ends remain open for attached plugs; screw-terminal ends remain open with {PD_TERMINAL_WIRE_SERVICE_L:.0f} mm validated wire-service envelopes
...
- A conservative {PD_ASSUMED_ASSEMBLY_DEPTH:.1f} mm card-assembly depth clears the bus-bar cover in the validated assembly
...
placement=f'''# USB-PD Card Placement — R0.12
...
- Carrier center: **X 0, Y +{PD_CARRIER_CENTER_Y:.1f} mm**.
- Battery card center: **X 0, Y {PD_CARRIER_CENTER_Y + PD_CARD_CENTERS_Y[0]:.1f} mm**.
- Starlink card center: **X 0, Y +{PD_CARRIER_CENTER_Y + PD_CARD_CENTERS_Y[1]:.1f} mm**.
...
- Keep at least **{PD_TERMINAL_WIRE_SERVICE_L:.0f} mm** clear beyond each screw-terminal end for conductors and bend relief.
...
- Automated clearance validation uses a conservative **{PD_ASSUMED_ASSEMBLY_DEPTH:.1f} mm** total card/component depth.
- Before energizing, close the lid without screws and confirm that no component, solder joint, or wire is being compressed.
'''
(OUT/'PD_CARD_PLACEMENT.md').write_text(placement)
