...
REV = 'R0.10'
OUT = Path(__file__).resolve().parent / 'MissionComHub_MkI_Enclosure_R0.10'
...
# heat-set-insert bosses on the underside of the lid; the R0.9 body is
# unchanged. Open-ended guide rails and paired zip-tie slots accommodate
...
parts={
 'MCH-MkI_body_R0.10':body,
 'MCH-MkI_lid_R0.10':lid,
 'MCH-MkI_dual_USB-PD_card_carrier_R0.10':pd_carrier,
 'MCH-MkI_peplink_strain_relief_R0.10':clamp,
 'MCH-MkI_control_fit_coupon_R0.10':control_coupon,
 'MCH-MkI_USB-C_final_fit_coupon_R0.10':usb_coupon,
 'MCH-MkI_wire_exit_fit_coupon_R0.10':wire_coupon,
 'MCH-MkI_BlueSea5045_fused_block_riser_coupon_R0.10':fused_block_coupon,
 'MCH-MkI_unfused_busbar_riser_coupon_R0.10':unfused_block_coupon,
}
...
shutil.copy(Path(__file__).resolve(),OUT/'make_mch_mk1_r10.py')
(OUT/'dimensions.json').write_text(json.dumps(manifest,indent=2))

readme=f'''# Mission Communications Hub Mk I Enclosure R0.10 — USB-PD Carrier
...
- Installs under the lid, centered at X=0, Y={PD_CARRIER_CENTER_Y:.1f} mm over the unfused bus bar
...
placement=f'''# USB-PD Card Placement — R0.10
...
- The carrier comes off with four M3 screws; the body does not need to be reprinted.

## Lid preparation

1. Print the R0.10 lid branding-side upward.
