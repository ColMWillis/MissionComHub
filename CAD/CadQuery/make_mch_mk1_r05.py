...
REV = 'R0.5'
OUT = Path(__file__).resolve().parent / 'MissionComHub_MkI_Enclosure_R0.5'
...
PEPLINK_SLOT_W = 4.35
PEPLINK_SLOT_H = 2.65
PEPLINK_CHANNEL_W = 4.05
PEPLINK_CHANNEL_H = 2.20
...
parts={
 'MCH-MkI_body_R0.5':body,
 'MCH-MkI_lid_R0.5':lid,
 'MCH-MkI_peplink_strain_relief_R0.5':clamp,
 'MCH-MkI_control_fit_coupon_R0.5':control_coupon,
 'MCH-MkI_AAOTOKK_USB-C_fit_coupon_R0.5':usb_coupon,
 'MCH-MkI_wire_exit_fit_coupon_R0.5':wire_coupon,
}
...
shutil.copy(Path(__file__).resolve(),OUT/'make_mch_mk1_r05.py')
(OUT/'dimensions.json').write_text(json.dumps(manifest,indent=2))

readme=f'''# Mission Communications Hub Mk I Enclosure R0.5
...
R0.5 widens the R0.4 voltmeter opening by 1.00 mm total (0.50 mm on the left and right) while preserving the verified 26.39 mm height.
...
3. Adjust the parameters at the top of `make_mch_mk1_r05.py` if necessary.
...
## Serviceability
- The R0.4 4 mm removable lid, corrected locating lip, M3 corner-boss pattern, and raised branding are preserved.
...
```text
python make_mch_mk1_r05.py
```

The generator writes a synchronized STEP, STL, and millimeter-unit 3MF for each part into a `MissionComHub_MkI_Enclosure_R0.5` folder beside the script.
'''
