...
STL_DIR = REPO / "CAD/STL/R0.6"
MF3_DIR = REPO / "CAD/3MF/R0.6"
STEP_DIR = REPO / "CAD/STEP/R0.6"
RELEASE_DIR = REPO / "CAD/Releases/R0.6"
...
results = {"revision": "R0.6", "files": {}}
manifest = json.loads((RELEASE_DIR / "dimensions.json").read_text())
if manifest.get("voltmeter_cutout_mm") != [45.17, 26.39]:
    raise RuntimeError(f"Unexpected R0.6 voltmeter cutout: {manifest.get('voltmeter_cutout_mm')}")
results["voltmeter_cutout_mm"] = manifest["voltmeter_cutout_mm"]
if manifest.get("peplink_wire_exit_mm") != [4.1, 2.4]:
    raise RuntimeError(f"Unexpected R0.6 Peplink exit: {manifest.get('peplink_wire_exit_mm')}")
if manifest.get("peplink_strain_channel_mm") != [4.55, 2.7]:
    raise RuntimeError(f"Unexpected R0.6 strain channel: {manifest.get('peplink_strain_channel_mm')}")
...
body_extent = results["files"]["MCH-MkI_body_R0.6"]["extents_mm"]
if not np.allclose(body_extent, [140.0, 90.0, 52.0], atol=0.02):
    raise RuntimeError(f"Body envelope is incorrect: {body_extent}")

body_step = cq.importers.importStep(str(STEP_DIR / "MCH-MkI_body_R0.6.step"))
lid_step = cq.importers.importStep(str(STEP_DIR / "MCH-MkI_lid_R0.6.step")).translate((0, 0, 52.0))
...
lines = [
    "# R0.6 Validation Report",
    "",
    "All matching STL, 3MF, and STEP exports passed automated validation.",
    "",
    "R0.6 preserves the verified **45.17 × 26.39 mm** voltmeter cutout.",
    "",
