...
STL_DIR = REPO / "CAD/STL/R0.12"
MF3_DIR = REPO / "CAD/3MF/R0.12"
STEP_DIR = REPO / "CAD/STEP/R0.12"
RELEASE_DIR = REPO / "CAD/Releases/R0.12"
...
results = {"revision": "R0.12", "files": {}}
manifest = json.loads((RELEASE_DIR / "dimensions.json").read_text())
if manifest.get("voltmeter_cutout_mm") != [45.17, 26.39]:
    raise RuntimeError(f"Unexpected R0.12 voltmeter cutout: {manifest.get('voltmeter_cutout_mm')}")
results["voltmeter_cutout_mm"] = manifest["voltmeter_cutout_mm"]
if manifest.get("peplink_wire_exit_mm") != [4.1, 2.4]:
    raise RuntimeError(f"Unexpected R0.12 Peplink exit: {manifest.get('peplink_wire_exit_mm')}")
if manifest.get("peplink_strain_channel_mm") != [4.55, 2.7]:
    raise RuntimeError(f"Unexpected R0.12 strain channel: {manifest.get('peplink_strain_channel_mm')}")
...
if carrier_data.get("carrier_mm") != [64.0, 54.0, 2.4]:
    raise RuntimeError(f"Unexpected PD carrier envelope: {carrier_data.get('carrier_mm')}")
if carrier_data.get("layout") != "side_by_side_parallel_usb_positive_x":
    raise RuntimeError(f"Unexpected PD carrier layout: {carrier_data.get('layout')}")
if carrier_data.get("guide_inward_change_total_mm") != 2.0:
    raise RuntimeError("PD guide rails were not moved inward by 2.0 mm total")
if carrier_data.get("guide_height_mm") != 3.6:
    raise RuntimeError("PD guide rails were not raised to 3.6 mm")
...
body_extent = results["files"]["MCH-MkI_body_R0.12"]["extents_mm"]
...
body_step = cq.importers.importStep(str(STEP_DIR / "MCH-MkI_body_R0.12.step"))
lid_step = cq.importers.importStep(str(STEP_DIR / "MCH-MkI_lid_R0.12.step")).translate((0, 0, body_height))
...
carrier_step = cq.importers.importStep(
    str(STEP_DIR / "MCH-MkI_dual_USB-PD_card_carrier_R0.12.step")
)
carrier_installed = (
...
rail_tip_z = carrier_plate_bottom_z - carrier_data["guide_height_mm"]
board_bottom_z = rail_tip_z - carrier_data["assumed_max_card_assembly_depth_mm"]
board_proxies = []
...
# Validate the real-world cable directions observed during fit testing. Both
# USB-C plugs extend toward +X, while the power leads leave the open screw-
# terminal ends toward -X. These conservative rectangular service envelopes
# must stay inside the body and clear both power blocks.
plug_l, plug_w, plug_h = carrier_data["attached_usb_plug_envelope_mm"]
card_l = carrier_data["card_envelope_each_mm"][0]
wire_l = carrier_data["terminal_wire_service_length_mm"]
plug_proxies = []
wire_proxies = []
for center_x, center_y in carrier_data["card_centers_in_enclosure_xy_mm"]:
    plug_proxies.append(
        cq.Workplane("XY")
        .box(plug_l, plug_w, plug_h, centered=(True, True, False))
        .translate((center_x + card_l / 2 + plug_l / 2, center_y, board_bottom_z))
    )
    wire_proxies.append(
        cq.Workplane("XY")
        .box(wire_l, 8.0, 8.0, centered=(True, True, False))
        .translate((center_x - card_l / 2 - wire_l / 2, center_y, board_bottom_z))
    )
service_interference = {}
for name, proxies in (("usb_plugs", plug_proxies), ("terminal_wires", wire_proxies)):
    service_interference[f"{name}_to_body_mm3"] = sum(
        sum(s.Volume() for s in body_step.intersect(proxy).solids().vals()) for proxy in proxies
    )
    service_interference[f"{name}_to_fused_block_mm3"] = sum(
        sum(s.Volume() for s in fuse_proxy.intersect(proxy).solids().vals()) for proxy in proxies
    )
    service_interference[f"{name}_to_busbar_mm3"] = sum(
        sum(s.Volume() for s in bus_proxy.intersect(proxy).solids().vals()) for proxy in proxies
    )
if any(volume > 0.05 for volume in service_interference.values()):
    raise RuntimeError(f"PD plug/wire service-envelope interference: {service_interference}")
results["pd_plug_and_wire_service_interference_mm3"] = {
    name: round(float(volume), 6) for name, volume in service_interference.items()
}
inner_half_x = (manifest["enclosure_mm"][0] - 2 * manifest["wall_mm"]) / 2
plug_max_x = card_l / 2 + plug_l
wire_min_x = -(card_l / 2 + wire_l)
results["attached_usb_plug_to_inner_wall_clearance_mm"] = round(float(inner_half_x - plug_max_x), 3)
results["terminal_wire_envelope_to_inner_wall_clearance_mm"] = round(float(wire_min_x + inner_half_x), 3)
