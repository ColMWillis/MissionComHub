...
STL_DIR = REPO / "CAD/STL/R0.10"
MF3_DIR = REPO / "CAD/3MF/R0.10"
STEP_DIR = REPO / "CAD/STEP/R0.10"
RELEASE_DIR = REPO / "CAD/Releases/R0.10"
...
results = {"revision": "R0.10", "files": {}}
manifest = json.loads((RELEASE_DIR / "dimensions.json").read_text())
if manifest.get("voltmeter_cutout_mm") != [45.17, 26.39]:
    raise RuntimeError(f"Unexpected R0.10 voltmeter cutout: {manifest.get('voltmeter_cutout_mm')}")
results["voltmeter_cutout_mm"] = manifest["voltmeter_cutout_mm"]
if manifest.get("peplink_wire_exit_mm") != [4.1, 2.4]:
    raise RuntimeError(f"Unexpected R0.10 Peplink exit: {manifest.get('peplink_wire_exit_mm')}")
if manifest.get("peplink_strain_channel_mm") != [4.55, 2.7]:
    raise RuntimeError(f"Unexpected R0.10 strain channel: {manifest.get('peplink_strain_channel_mm')}")
...
body_extent = results["files"]["MCH-MkI_body_R0.10"]["extents_mm"]
if not np.allclose(body_extent, [146.0, 96.0, 52.0], atol=0.02):
    raise RuntimeError(f"Body envelope is incorrect: {body_extent}")

body_step = cq.importers.importStep(str(STEP_DIR / "MCH-MkI_body_R0.10.step"))
lid_step = cq.importers.importStep(str(STEP_DIR / "MCH-MkI_lid_R0.10.step")).translate((0, 0, 52.0))
...
carrier_step = cq.importers.importStep(
    str(STEP_DIR / "MCH-MkI_dual_USB-PD_card_carrier_R0.10.step")
)
carrier_installed = (
    carrier_step.rotate((0, 0, 0), (1, 0, 0), 180)
    .translate((0, carrier_data["carrier_center_in_enclosure_xy_mm"][1], 52.0 - carrier_data["lid_boss_height_mm"]))
)
carrier_clearances = {
...
carrier_plate_bottom_z = 52.0 - carrier_data["lid_boss_height_mm"] - carrier_data["carrier_mm"][2]
...
lines = [
    "# R0.10 USB-PD Carrier Validation Report",
    "",
    "All matching STL, 3MF, and STEP exports passed automated validation.",
    "",
    "R0.10 preserves the verified **45.17 × 26.39 mm** voltmeter cutout.",
    "",
...
lines += [
    ...
    "",
    f"The conservative 8 mm-deep PD-card envelopes retain **{results['pd_card_to_busbar_vertical_clearance_mm']:.3f} mm** vertical clearance above the bus-bar cover.",
    "",
    "Checks performed: watertightness, winding consistency, positive volume, 3MF package structure and millimeter units, STL/3MF extent agreement, STEP re-import, the 146 × 96 × 52 mm body envelope, body/lid interference, power-block envelope clearance, installed carrier clearance, and conservative PD-card envelope clearance.",
]
